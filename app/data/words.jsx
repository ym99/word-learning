export const words = [
    /* 001 */ { spanish: 'gracias', english: 'thanks' },
    /* 002 */ { spanish: 'ser', english: 'to be + somebody' },
    /* 003 */ { onlyToEnglish: true, spanish: 'a', english: 'to' },
    /* 004 */ { spanish: 'ir', english: 'to go' },
    /* 005 */ { spanish: 'estar', english: 'to be + location, feeling' },
    /* 006 */ { spanish: 'bueno', english: 'good' },
    /* 007 */ { spanish: 'de', english: 'of' },
    /* 008 */ { spanish: 'su', english: ['your', 'her', 'his', 'their'] },
    /* 009 */ { spanish: 'hacer', english: ['to do', 'to make'] },

    /* 010 */ { spanish: 'amigo', english: 'friend' },
    /* 011 */ { spanish: 'por favor', english: 'please' },
    /* 012 */ { spanish: 'no', english: 'no' },
    /* 013 */ { spanish: 'en', english: ['on', 'in'] },
    /* 014 */ { spanish: 'haber', english: 'to have + auxiliary, supportive verb' },
    /* 015 */ { spanish: 'tener', english: ['to have + to possess', 'to possess'] },
    /* 016 */ { spanish: ['un', 'uno', 'una'], english: ['a', 'one'] },
    /* 017 */ { spanish: 'ahora', english: 'now' },
    /* 018 */ { spanish: 'y', english: 'and' },
    /* 019 */ { spanish: 'qué', english: 'what' },

    /* 020 */ { onlyToEnglish: true, spanish: 'por', english: ['for', 'by'] },
    /* 021 */ { spanish: 'amar', english: 'to love' },
    /* 022 */ { spanish: 'quién', english: 'who' },
    /* 023 */ { onlyToEnglish: true, spanish: 'para', english: ['for', 'to'] },
    /* 024 */ { spanish: 'venir', english: 'to come' },
    /* 025 */ { spanish: 'porque', english: 'because' },
    /* 026 */ { spanish: ['el', 'la', 'los', 'las'], english: 'the' },
    /* 027 */ { spanish: 'antes', english: 'before' },
    /* 028 */ { spanish: 'más', english: 'more' },
    /* 029 */ { spanish: 'bien', english: 'well' },

    /* 030 */ { spanish: ['aqui', 'alli'], english: ['here', 'there'] },
    /* 031 */ { spanish: 'querer', english: 'to want' },
    /* 032 */ { spanish: 'hola', english: 'hello' },
    /* 033 */ { spanish: 'tú', english: 'you + singular' },
    /* 034 */ { spanish: 'poder', english: 'to be able' },
    /* 035 */ { spanish: 'gustar', english: ['to be pleasing', 'to like'] },
    /* 036 */ { spanish: 'poner', english: 'to put' },
    /* 037 */ { spanish: 'casi', english: 'almost' },
    /* 038 */ { spanish: 'saber', english: 'to know + fact or skill' },
    /* 039 */ { spanish: 'como', english: ['like', 'as'] },

    /* 040 */ { spanish: 'donde', english: 'where' },
    /* 041 */ { spanish: 'dar', english: 'to give' },
    /* 042 */ { spanish: 'pero', english: 'but' },
    /* 043 */ { spanish: 'se', english: ['itself', 'herself', 'himself', 'themselves'] },
    /* 044 */ { spanish: 'mucho', english: 'much' },
    /* 045 */ { spanish: 'nuevo', english: 'new' },
    /* 046 */ { spanish: 'cuando', english: 'when' },
    /* 047a */ { spanish: 'chico', english: 'boy' },
    /* 047b */ { spanish: 'chica', english: 'girl' },
    /* 048 */ { spanish: ['entender', 'comprender'], english: 'to understand' },
    /* 049 */ { spanish: 'si', english: ['if', 'yes'] },

    /* 050 */ { spanish: 'o', english: 'or' },
    /* 051 */ { spanish: 'feliz', english: 'happy' },
    /* 052 */ { spanish: 'todo', english: ['all', 'every'] },
    /* 053 */ { spanish: 'mismo', english: 'same' },
    /* 054 */ { spanish: 'muy', english: 'very' },
    /* 055 */ { spanish: 'nunca', english: 'never' },
    /* 056 */ { spanish: 'yo', english: 'I' },
    /* 057 */ // coded in 049:si -> yes
    /* 058a */ { spanish: 'grande', english: 'big' },
    /* 058b */ { spanish: 'gran', english: 'great' },
    /* 059 */ { spanish: 'deber', english: ['to owe', 'should'] },

    /* 060 */ { spanish: 'usted', english: 'you + plural' },
    /* 061 */ { spanish: 'bajo', english: ['low', 'under'] },
    /* 062 */ { spanish: 'otro', english: 'other' },
    /* 063 */ { spanish: 'salir', english: 'to leave' },
    /* 064 */ { spanish: 'hora', english: 'hour' },
    /* 065 */ { spanish: 'desde', english: 'from' },
    /* 066 */ { spanish: 'ver', english: ['to see', 'to watch'] },
    /* 067 */ { spanish: ['malo', 'mal'], english: 'bad' },
    /* 068 */ { spanish: 'pensar', english: 'to think' },
    /* 069 */ { spanish: 'hasta', english: 'until' },

    /* 070 */ // ['tanto', 'tan'], '?', ' used in making comparison' },
    /* 071 */ { spanish: 'entre', english: ['between', 'among'] },
    /* 072 */ { spanish: 'durante', english: 'during' },
    /* 073 */ { spanish: 'llevar', english: ['to wear', 'to carry'] },
    /* 074 */ { spanish: 'siempre', english: 'always' },
    /* 075 */ { spanish: 'empezar', english: 'to begin' },
    /* 076a */ { spanish: 'él', english: 'he' },
    /* 076b */ { spanish: 'ella', english: 'she' },
    /* 076c */ { spanish: ['ellos', 'ellas'], english: 'they' },
    /* 077 */ { spanish: 'leer', english: 'to read' },
    /* 078 */ { spanish: 'cosa', english: 'thing' },
    /* 079 */ { spanish: 'sacar', english: ['to take out', 'to remove'] },

    /* 080 */ { spanish: 'conocer', english: 'to know + person or place' },
    /* 081 */ { spanish: 'primero', english: 'first' },
    /* 082 */ { spanish: 'andar', english: 'to walk + when enjoying ...' },
    /* 083 */ { spanish: 'sobre', english: ['over', 'about'] },
    /* 084 */ { spanish: 'echar', english: 'to throw' },
    /* 085 */ { spanish: 'sin', english: 'without' },
    /* 086 */ { spanish: 'decir', english: 'to say' },
    /* 087 */ { spanish: 'trabajar', english: 'to work' },
    /* 088 */ { spanish: 'nosotros', english: 'we' },
    /* 089 */ { spanish: 'tambien', english: 'also' },

    /* 090 */ { spanish: 'adiós', english: 'goodbye' },
    /* 091 */ { spanish: 'comer', english: 'to eat' },
    /* 092 */ { spanish: 'triste', english: 'sad' },
    /* 093 */ { spanish: 'pais', english: 'country' },
    /* 094 */ { spanish: 'escuchar', english: 'to listen' },
    /* 095 */ { spanish: 'hombre', english: 'man' },
    /* 096 */ { spanish: 'mujer', english: 'woman' },
    /* 097 */ // le
    /* 098 */ { spanish: 'creer', english: ['to believe', 'to think + (to believe)'] },
    /* 099 */ { spanish: 'encontrar', english: 'to find' },

    /* 100 */ { spanish: 'beber', english: 'to drink' },

    { spanish: 'mirar', english: 'to look' },
    { spanish: 'hablar', english: 'to talk' },
    { spanish: 'sentir', english: 'to feel' },
    { spanish: 'oir', english: 'to hear' },
    { spanish: 'rapido', english: 'fast' },
    { spanish: 'lento', english: 'slow' },
    { spanish: 'tuerte', english: 'strong + when speaking' },
    { spanish: 'suave', english: 'soft' },
    { spanish: 'caro', english: 'car' },

    { spanish: 'caminar', english: 'to walk + to some location' },
    { spanish: 'plátano', english: 'banana' },
    { spanish: 'respuesta', english: 'answer' },
    { spanish: 'esta', english: 'this + you can touch it' },
    { spanish: 'esa', english: 'that + you can\'t touch it' },
    { spanish: 'con', english: 'with' },
    { spanish: 'salud', english: 'health' },
    { spanish: 'perfecta', english: 'perfect' },

    { spanish: 'dulce', english: 'sweet' },
    { spanish: 'salado', english: 'salty' },
    { spanish: 'sabroso', english: 'tasty' },
    { spanish: 'amargo', english: 'bitter' },
    { spanish: 'agrio', english: 'sour' },

    { spanish: 'huevo', english: 'egg' },
    { spanish: 'leche', english: 'milk' },
    { spanish: 'almuerzo', english: 'lunch' },
    { spanish: 'desayuno', english: 'breakfast' },
    { spanish: 'cereal', english: 'cereal' },
    { spanish: 'salchicha', english: 'sausage' },
    { spanish: 'tocino', english: 'bacon' },
    { spanish: 'yogur', english: 'yogurt' },
    { spanish: 'fresa', english: 'strawberry' },
    { spanish: 'hamburguesa', english: 'hamburger' },
    { spanish: 'jamón', english: 'ham' },
    { spanish: 'manzana', english: 'apple' },
    { spanish: 'cheese', english: 'queso' },

    { spanish: 'comida', english: 'food' },
    { spanish: 'galleta', english: 'cookie' },
    { spanish: 'limón', english: 'lemon' },
    { spanish: 'espaguetis', english: 'spaghetti' },
    { spanish: 'jugo', english: 'juice' },
    { spanish: 'pescado', english: 'fish' },
    { spanish: 'arroz', english: 'rice' },
    { spanish: 'tomate', english: 'tomato' },
    { spanish: 'cebolla', english: 'onion' },
    { spanish: 'pollo', english: 'chicken' },
    { spanish: 'carne', english: 'meat' },
    { spanish: 'mantequilla', english: 'butter' },
    { spanish: 'ensalada', english: 'salad' },
    { spanish: 'calorías', english: 'calories' },
    { spanish: 'grasa', english: 'grease' },
    { spanish: 'sal', english: 'salt' },
    { spanish: 'tostada', english: 'toast' },

    { spanish: 'enfermo', english: 'sick' },
    { spanish: 'cansado', english: 'tired' },
    { spanish: 'triste', english: 'sad' },
    { spanish: 'sediento', english: 'thirsty' },
    { spanish: 'hambriento', english: 'hungry' },
    { spanish: 'mejor', english: 'best' },

    { spanish: 'pelar', english: 'to peel' },
    { spanish: 'necesitar', english: 'to need' },

    { spanish: 'logar', english: 'place' },
    { spanish: 'plaza', english: 'square' },
    { spanish: 'oficina', english: 'office' },
    { spanish: 'casa', english: 'home' },
    { spanish: 'parque', english: 'park' },
    { spanish: 'restaurante', english: 'restaurant' },
    { spanish: 'baño', english: 'bathroom' },
    { spanish: 'biblioteca', english: 'library' },
    { spanish: 'gimnasio', english: 'fitness center' },
    { spanish: 'escuela', english: 'school' },
    { spanish: 'museo', english: 'museum' },
    { spanish: 'cine', english: ['cinema', 'movie theater'] },
    { spanish: 'cocina', english: 'kitchen' },

    { spanish: ['banito', 'banita'], english: 'pretty' },
    { spanish: 'quedar', english: 'to stay' },
    { spanish: 'viajar', english: 'to travel' },
    { spanish: 'buscar', english: 'to search' },
    { spanish: 'encontrar', english: 'to find' },
    { spanish: 'cocinar', english: 'to cook' },

    { spanish: 'sala', english: 'living room' },
    { spanish: 'cuarto', english: 'room' },
    { spanish: 'sótano', english: 'basement' },
    { spanish: 'teatro', english: 'theater' },
    { spanish: 'ciudad', english: 'city' },
    { spanish: 'iglesia', english: 'church' },

    { spanish: 'jugar', english: 'to play + sport' },
    { spanish: 'tocar', english: 'to play + musical instrument' },
    { spanish: 'escalera', english: 'stairs' },
    { spanish: 'conocimiento', english: 'knowledge' },
    { spanish: 'junto', english: 'together' },
    { spanish: 'padre', english: 'father' },
    { spanish: 'mother', english: 'madre' },
    { spanish: 'papá', english: 'dad' },
    { spanish: 'mamá', english: 'mom' },
    { spanish: 'hermano', english: 'brother' },
    { spanish: 'hermana', english: 'sister' },
    { spanish: 'hijo', english: 'son' },
    { spanish: 'hija', english: 'daughter' },
    { spanish: ['primo', 'prima'], english: 'cousin' },
    { spanish: 'abuelo', english: 'grandfather' },
    { spanish: 'abuela', english: 'grandmother' },

    { onlyToEnglish: true, russian: 'поросёнок', english: 'little pig' },
    { onlyToEnglish: true, russian: 'красавица', english: 'beauty' },

    { spanish: 'compartir', english: 'share' },
    { spanish: 'contigo', english: 'with you' },
    { spanish: 'cena', english: 'dinner' },
    { spanish: 'minuto', english: 'minute' },
    { spanish: 'semana', english: 'week' },
    { spanish: 'fecha', english: 'date' },
    { spanish: 'mes', english: 'month' },
    { spanish: 'año', english: 'year' },
    { spanish: 'cuántos', english: 'how many' },
    { spanish: 'cual', english: 'which' },
    { spanish: 'buenos días', english: 'good morning' },
    { spanish: 'buena noches', english: 'good evening' },
    { spanish: 'buenas noches', english: 'goodnight' },
    { spanish: 'buenas tardes', english: 'good afternoon' },
    { spanish: 'nombre', english: 'name' },
    { spanish: 'llamar', english: 'to call' },
    { spanish: 'ayuda', english: 'help' },
    { spanish: 'ayudar', english: 'to help' },
    { spanish: 'escribir', english: 'to write' },
    { spanish: 'playa', english: 'beach' },
    { spanish: 'vacaciones', english: 'vacation' },
    { spanish: 'pescar', english: 'to fish' },
    { spanish: 'viaje', english: 'trip' },
    { spanish: 'preparar', english: 'to prepare' },
    { spanish: 'fin de semana', english: 'weekend' },
    { spanish: 'nadar', english: 'to swim' },

    { spanish: 'caer', english: 'to fall' },
    { spanish: 'traer', english: 'to bring' },
    { spanish: 'ayer', english: 'yesterday' },
    { spanish: 'vamos', english: 'come on' },
    { spanish: 'trompeta', english: 'trumpet' },
    { spanish: 'hoy', english: 'today' },
    { spanish: 'en punto', english: 'o\'clock' },
    { spanish: 'anoche', english: 'last night' },
    { spanish: 'en la mañana', english: 'morning' },
    { spanish: 'mañana', english: 'tomorrow' },
    { spanish: 'otro día', english: 'other day' },
    { spanish: 'pasado', english: 'past + last' },

    { spanish: 'proximo', english: 'next' },
    { spanish: 'tarde', english: 'afternoon' },
    { spanish: 'después', english: 'after' },
    { spanish: 'invitar', english: 'to invite' },

    { spanish: 'abordar', english: 'to board' },
    { spanish: 'abrazar', english: 'to embrace' },
    { spanish: 'abrir', english: 'to open' },
    { spanish: 'absolver', english: 'to absolve' },
    { spanish: 'acabar de', english: 'to have just' },
    { spanish: 'acabarse', english: 'to use up' },
    { spanish: 'acercarse', english: 'to approach' },
    { spanish: 'aclarar', english: 'to clarify' },
    { spanish: 'acompanar', english: 'to accompany' },
    { spanish: 'aconsejar', english: 'to adivise' },
    { spanish: 'acordarse', english: 'to remember' },
    { spanish: 'acostarse', english: 'to go to bed' },
    { spanish: 'acostumbrar', english: 'to be accustomed' },
    { spanish: 'actuar', english: 'to act' },
    { spanish: 'acusar', english: 'to accuse' },
    { spanish: 'adivinar', english: 'to guess' },

    // ----------------------------------------------------------------------------
    // part 1
    { spanish: 'acampar', english: 'to camp' },
    { spanish: 'el alojamiento', english: 'the lodging' },
    { spanish: 'el anillo', english: 'the ring' },
    { spanish: 'el año pasado', english: 'last year' },
    { spanish: 'anteayer', english: 'the day before yesterday' },
    { spanish: 'el arete', english: 'the earring' },
    { spanish: 'las artesanias', english: 'the handcrafts' },
    { spanish: 'el ascensor', english: 'the elevator' },
    { spanish: 'bello', english: ['beautiful', 'nice'] },
    { spanish: 'caro', english: 'expensive' },

    { spanish: 'el collar', english: 'the necklace' },
    { spanish: 'dar una caminata', english: 'to hike' },
    { spanish: 'demasiado', english: ['too', 'too much'] },
    { spanish: 'el dinero en efectivo', english: 'the cash' },
    { spanish: 'el fue', english: ['he went', 'he was'] },
    { spanish: 'ella fue', english: ['she went', 'she was'] },
    { spanish: 'ellas fueron', english: 'they went + all females' },
    { spanish: 'ellos fueron', english: 'they went' },
    { spanish: 'el turista', english: 'the tourist + male' },
    { spanish: 'la turista', english: 'the tourist + female' },
    { spanish: 'estar de vacaciones', english: 'to be on vacation' },
    { spanish: 'la habitación', english: 'the hotel room' },
    { spanish: 'la habitación doble', english: 'the double room' },
    { spanish: 'la habitación individual', english: 'the single room' },

    { spanish: 'hacer una excursión', english: 'to go on a day trip' },
    { spanish: 'hacer una reservación', english: 'to make a reservation' },
    { spanish: 'el hostal', english: 'the hostel' },
    { spanish: 'el hotel', english: 'the hotel' },
    { spanish: 'las joyas', english: 'the jewerly' },
    { spanish: 'le dejo X en Y', english: 'I\'ll give X to you for Y' },
    { spanish: 'la llave', english: 'the key' },
    { spanish: 'mandar tarjetas postales', english: 'to send postcards' },
    { spanish: 'me gustaria', english: 'I would like' },
    { spanish: 'el mercado al aire libre', english: 'the open-air market' },
    { spanish: 'el mes pasado', english: 'last month' },
    { spanish: 'montar a caballo', english: 'to ride a horse' },
    { spanish: 'nosotros fuimos', english: 'we went' },

    { spanish: '¿Podria ver X?', english: ['Could I see X?', 'could I look at X?'] },
    { spanish: '¡Qué adjetivo!', english: 'How adjective!' },
    { spanish: '¡Qué bello!', english: 'How beautiful!' },
    { spanish: '¡Qué caro!', english: 'So expensive!' },
    { spanish: 'la recepción', english: 'the reception desk' },
    { spanish: 'el recuerdo', english: 'the souvenir' },
    { spanish: 'regatear', english: 'to bargain' },
    { spanish: 'la semana pasada', english: 'last week' },
    { spanish: 'la tarjeta de crédito', english: 'the credit card' },
    { spanish: 'la tarjeta postal', english: 'postcard' },
    { spanish: 'tener una reservación', english: 'to have a reservation' },
    { spanish: 'el tiempo libre', english: 'the free time' },
    { spanish: 'tomar fotos', english: 'to take photos' },
    { spanish: 'tú fuiste', english: ['you went', 'you were'] },
    { spanish: 'ustedes fueron', english: ['you all went + formal', 'you all were + formal'] },
    { spanish: 'usted fue', english: ['you went + formal', 'you were + formal'] },

    { spanish: 'ver las atracciones', english: 'to see sightseeing' },
    { spanish: 'visitar un museo', english: 'to visit a museum' },
    { spanish: 'vosotros fuisteis', english: ['you all went', 'you all were'] },
    { spanish: 'yo fui', english: ['I went', 'I was'] },

    { spanish: 'el campeonato', english: 'the championship' },
    { spanish: 'la competencia', english: 'the competition' },
    { spanish: 'el campeón', english: 'the champion  + male' },
    { spanish: 'la campeóna', english: 'the champion + female' },
    { spanish: 'el trofeo', english: 'the trophy' },
    { spanish: 'una medalla', english: 'a medal' },
    { spanish: 'el ciclismo', english: 'bicycle racing' },
    { spanish: 'aprender a', english: 'to learn to' },
    { spanish: 'salir campeón', english: 'to be crowned champion + male' },
    { spanish: 'salir campeóna', english: 'to be crowned champion + female' },
    { spanish: 'recibir el premio', english: 'to receive the prize' },
    { spanish: 'ganar', english: 'to win' },
    { spanish: 'perder', english: 'to lose' },
    { spanish: 'estar empatado', english: 'to be tied' },
    { spanish: 'empatar', english: 'to tie + score' },
    { spanish: 'gritar cantos deportivos', english: 'to shout sport chants' },
    { spanish: 'meter un gol', english: 'to score a goal' },
    { spanish: 'jugar en equipo', english: 'to play on a team' },

    { spanish: 'el bate', english: 'the bat' },
    { spanish: 'el campo', english: 'the field' },
    { spanish: 'la cancha', english: 'the court' },
    { spanish: 'el casco', english: 'the helmet' },
    { spanish: 'la pista', english: 'the track' },
    { spanish: 'la raqueta', english: 'the racket' },
    { spanish: 'la red', english: 'the net' },
    { spanish: 'el uniforme', english: 'the uniform' },

    { spanish: '¡Ay, por favor!', english: 'Oh, please!' },
    { spanish: '¡Bravo!', english: 'Bravo!' },
    { spanish: '¡Dale!', english: 'Come on!' },
    { spanish: '¡Uy!', english: 'Ugh!' },

    { spanish: 'el deporte', english: 'the sport' },
    { spanish: 'La Copa Mundial', english: 'The World Cup' },
    { spanish: 'Los Juegos Olímpicos', english: 'The Olympic Games' },
    { spanish: 'Los Juegos Panamericanos', english: 'The Panamerican games' },
    { spanish: 'La Vuelta a Francia', english: 'The Tour de France' },

    { spanish: ['el atleta', 'el deportista'], english: 'the athlete + male' },
    { spanish: ['la atleta', 'la deportista'], english: 'the athlete + female' },
    { spanish: 'atlético', english: 'athletic + male' },
    { spanish: 'atlética', english: 'athletic + female' },
    { spanish: 'activo', english: 'active + male' },
    { spanish: 'activa', english: 'active + female' },
    { spanish: 'deportivo', english: ['sport + male', 'sporty  + male', 'used for sports + male'] },
    { spanish: 'deportiva', english: ['sport + female', 'sporty + female', 'used for sports + female'] },
    { spanish: 'lento', english: 'slow + male' },
    { spanish: 'lenta', english: 'slow + female' },
    { spanish: 'musculoso', english: 'muscular + male' },
    { spanish: 'musculosa', english: 'muscular + female' },
    { spanish: 'rápido', english: 'fast + male' },
    { spanish: 'rápida', english: 'fast + female' },
    { spanish: 'serio', english: 'serious + male' },
    { spanish: 'seria', english: 'serious + female' },

    { spanish: 'es bueno', english: 'it is good to' },
    { spanish: 'es importante', english: 'it is important' },
    { spanish: 'es necesario', english: 'it is necessary' },

    { spanish: 'hacer ejercicio', english: 'to do exercise' },
    { spanish: 'estar en forma', english: 'to be in shape' },
    { spanish: 'comer comida sana', english: 'to eat healthy food' },
    { spanish: 'evitar las grasas', english: 'to avoid fatty foods' },
    { spanish: 'seguir una dieta balanceada', english: 'to follow a balanced diet' },
    { spanish: 'tener motivacíon', english: 'to have motivation' },
    { spanish: 'la proteína', english: 'the protein' },
    { spanish: 'los carbohidratos', english: 'carbohydrates' },
    { spanish: 'la energía', english: 'energy' },

    // ----------------------------------------------------------------------------
    // part 2
    { spanish: 'acostarse', english: 'to go to bed' },
    { spanish: 'afeitarse', english: 'to shave' },
    { spanish: 'apagar la luz', english: 'to turn off the light' },
    { spanish: 'arreglarse', english: 'to get ready' },
    { spanish: 'a veces', english: 'sometimes' },
    { spanish: 'bañarse', english: 'to take a bath' },
    { spanish: 'la cara', english: 'face' },
    { spanish: 'cepillarse los dientes', english: 'to brush one\'s teeth' },
    { spanish: 'el cepillo', english: 'brush' },
    { spanish: 'el cepillo de dientes', english: 'toothbrush' },
    { spanish: 'ducharse', english: 'to take a shower' },
    { spanish: 'ella se arregla', english: 'she gets ready' },
    { spanish: 'ellos se despiertan', english: 'they wake up' },
    { spanish: 'encender la luz', english: 'to turn on the lights' },
    { spanish: 'entonces', english: 'then' },
    { spanish: 'entrenarse', english: 'to train' },
    { spanish: 'estás escribiendo', english: 'you are writing' },
    { spanish: 'estoy viajando', english: 'I am travling' },
    { spanish: 'frecuentemente', english: 'frequently' },
    { spanish: 'la garganta', english: 'throat' },
    { spanish: 'el peine', english: 'comb' },
    { spanish: 'ponerse la ropa', english: 'to put on cloths' },
    { spanish: 'por fin', english: 'finally' },
    { spanish: 'primero', english: 'first' },
    { spanish: 'la rutina', english: 'routine' },
    { spanish: 'el secador de pelo', english: 'hair dryer' },
    { spanish: 'secarse', english: 'to dry oneself' },
    { spanish: 'te afeitas', english: 'you shave' },
    { spanish: 'te estás poniendo la ropa', english: 'you are putting on clothes' },
    { spanish: 'tener prisa', english: 'to be in a hurry' },
    { spanish: 'el champú', english: 'shampoo' },
    { spanish: 'el codo', english: 'elbow' },
    { spanish: 'la crema de afeitar', english: 'shaving cream' },
    { spanish: 'el cuello', english: 'neck' },
    { spanish: 'el dedo', english: 'finger' },
    { spanish: 'el dedo de pie', english: 'toe' },
    { spanish: 'el desodorante', english: 'deodorant' },
    { spanish: 'despertarse', english: 'to wake up' },
    { spanish: 'el diente', english: 'tooth' },
    { spanish: 'dornirse', english: 'to fall asleep' },
    { spanish: 'generalmente', english: ['in general', 'generally'] },
    { spanish: 'el hombro', english: 'shoulder' },
    { spanish: 'el jabón', english: 'soap' },
    { spanish: 'lavarse', english: 'to wash oneself' },
    { spanish: 'levantarse', english: 'to get up' },
    { spanish: 'luego', english: 'later' },
    { spanish: 'maquillarse', english: 'to put on makeup' },
    { spanish: 'mas tarde', english: 'later on' },
    { spanish: 'me acuesto', english: 'I go to bed' },
    { spanish: 'me estoy duchando', english: 'I am taking a shower' },
    { spanish: 'me pongo la ropa', english: 'I put on clothes' },
    { spanish: 'la muñeca', english: 'wrist' },
    { spanish: 'normalmente', english: 'normally' },
    { spanish: 'nos bañamos', english: 'we take a bath' },
    { spanish: 'el oído', english: 'inner ear' },
    { spanish: 'la oreja', english: 'outer ear' },
    { spanish: 'nos cepilláis los dientes', english: 'you all brush your teeth' },
    { spanish: 'la pasta de dientes', english: 'toothpaste' },
    { spanish: 'peinarse', english: 'to comb someone hair' },
    { spanish: 'pensar + infinitivo', english: 'to plan to' },
    { spanish: 'tener sueño', english: 'to be sleepy' },
    { spanish: 'la toalla', english: 'towel' },
    { spanish: 'la uña', english: 'nail' },
    { spanish: 'ustedes se duermen', english: 'you all fall asleep' },
    { spanish: 'yo enciendo la luz', english: 'I turn on the lights' },
    { spanish: 'yo pienso ducharme luego', english: 'I plan to take a shower later' },

    { new: true, spanish: 'admitir', english: 'to admit' },
    { new: true, spanish: 'advertir', english: 'to warn' },
    { new: true, spanish: 'agotar', english: 'to exhaust' },
    { new: true, spanish: 'agradar', english: 'to be pleasing' },
    { new: true, spanish: 'agradecer', english: 'to thank' },
    { new: true, spanish: 'aguantar', english: 'to tolerate' },
    { new: true, spanish: 'ahorrar', english: 'to save' },
    { new: true, spanish: 'alcanzar', english: 'to reach' },
    { new: true, spanish: 'almorzar', english: 'to have lunch' },
    { new: true, spanish: 'alquilar', english: 'to rent' },
    { new: true, spanish: 'alzar', english: 'to lift' },
    { new: true, spanish: 'amenazar', english: 'to threaten' },
    { new: true, spanish: 'anadir', english: 'to add' },
    { new: true, spanish: 'animarse', english: 'to cheer up' },
    { new: true, spanish: 'anorar', english: 'to yearn' },
    { new: true, spanish: 'anular', english: 'to annual' },
    { new: true, spanish: 'anunciar', english: 'to announce' },
    { new: true, spanish: 'apagar', english: 'to turn off' },
    { new: true, spanish: 'aparecer', english: 'to appear' },
    { new: true, spanish: 'aparecer', english: 'to separate' },
    { new: true, spanish: 'apostar', english: 'to bet' },
    { new: true, spanish: 'apoyar', english: 'to support' },
    { new: true, spanish: 'apreciar', english: 'to appreciate' },
    { new: true, spanish: 'aprender', english: 'to learn' },
    { new: true, spanish: 'apresurarse', english: 'to hurry' },
    { new: true, spanish: 'apretar', english: 'to squeeze' },
    { new: true, spanish: 'aprobar', english: 'to pass' },
    { new: true, spanish: 'arrancar', english: 'to pull out' },
    { new: true, spanish: 'arreglar', english: 'to fix' },
    { new: true, spanish: 'arreglarse', english: 'to get ready' },
    { new: true, spanish: 'arriesgar', english: 'to risk' },
    { new: true, spanish: 'asegurar', english: 'to assure' },
    { new: true, spanish: 'asistir', english: 'to attend' },
    { new: true, spanish: 'asustarse', english: 'to become scared' },
    { new: true, spanish: 'atender', english: 'to attend' },
    { new: true, spanish: 'aterrizar', english: 'to land' },
    { new: true, spanish: 'atraer', english: 'to attract' },
    { new: true, spanish: 'atravesar', english: 'to cross' },
    { new: true, spanish: 'atreverse', english: 'to dare to' },
    { new: true, spanish: 'averiguar', english: 'to find out' },
    { new: true, spanish: 'bajarse', english: 'to get off' },
    { new: true, spanish: 'barrer', english: 'to sweep' },
    { new: true, spanish: 'basar', english: 'to base' },
    { new: true, spanish: 'besar', english: 'to kiss' },
    { new: true, spanish: 'borrar', english: 'to erase' },
    { new: true, spanish: 'bostezar', english: 'to yawn' },
    { new: true, spanish: 'broncear', english: 'to drink' },
    { new: true, spanish: 'caber', english: 'to fit' },
    { new: true, spanish: 'calentar', english: 'to warm' },
    { new: true, spanish: 'callarse', english: 'to be quiet' },
    { new: true, spanish: 'calmarse', english: 'to calm down' },
    { new: true, spanish: 'cambiar', english: 'to change' },
    { new: true, spanish: 'captar', english: 'to grasp' },
    { new: true, spanish: 'cargar', english: 'to load' },
    { new: true, spanish: 'castigar', english: 'to punish' },
    { new: true, spanish: 'cenar', english: 'to cine' },
    { new: true, spanish: 'cerrar', english: 'to close' },
    { new: true, spanish: 'charlar', english: 'to chat' },
    { new: true, spanish: 'cobrar', english: '' },
    { new: true, spanish: 'coger', english: 'to catch' },
    { new: true, spanish: 'colgar', english: 'to hang up' },
];
/*
    { new: true, spanish: '', english: '' },
    {
        hide: true,
        new: true,
        onlyToEnglish: true, onlyFromEnglish: true,
        spanish: 'word', spanish: 'word + comment', spanish: ['word', 'word'],
        english: '',
        russian: '',
    }

    á Alt + 0225
    é Alt + 0233
    í Alt + 0237
    ó Alt + 0243
    ú Alt + 0250
    ñ Alt + 0241
    ü Alt + 0252
    ¡ Alt + 0161
    ¿ Alt + 0191
*/
